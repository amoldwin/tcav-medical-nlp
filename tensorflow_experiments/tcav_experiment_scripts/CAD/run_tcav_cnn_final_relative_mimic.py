...
name = 'weekday_nocad_results_cnn_final_mimic' + ckpt.split('/')[-1] + str(layer_number)+'.json'
...
time.sleep(.2) 
act_generator = act_gen.ImageActivationGenerator(mymodel, source_dir, activation_dir, max_examples=1000, document_encoder = act_gen.cnn_encode)
...
import json
with open(scores_dir+name, 'w') as jfile:
   jfile.write(json.dumps(results))
utils_plot.plot_results(results, random_concepts=concepts, name=name)
