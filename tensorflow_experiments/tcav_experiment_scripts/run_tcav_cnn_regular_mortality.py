...
ckpt='cnn_berttokenize_mortality2.ckpt'
...
concepts = concepts + [x for x in os.listdir(concepts_dir) if x.startswith('filtered')]

scoresstr = []#''.join([fn for fn in os.listdir('cnn_scores') if 'distilbert_final_n2c2' in fn])
concepts = [concept for concept in concepts if not concept in scoresstr and not 'encode' in concept and not 'disch' in concept]
...
mytcav = tcav.TCAV(sess,
                  ...
                  # random_concepts = [concept for concept in os.listdir('../concept_listfiles') if concept.startswith('disch_random500')][:10])
...
import json
with open(scores_dir+'regular_mortality_filtered_' + ckpt.split('/')[-1]+'_' +str(layer_number)+'_'+str(num_random_exp)+'randexp.json', 'w') as jfile:
