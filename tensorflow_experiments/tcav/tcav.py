...
class TCAV(object):
  ...
  def get_direction_dir_sign(mymodel, act, cav, concept, class_id, example):
    ...
    print('get_direction_dir_sign time',time.time()-start)
    return dot_prod < 0
  ...
  def compute_tcav_score(mymodel,
    ...
    weekdays= weekdays + ['WEEKDAY_discharge_'+ x for x in weekdays ]
    start = time.time()
    count = 0
    ...
    if run_parallel:
      ...
      print('compute_tcav_score',time.time()-start)
      ...
      print('compute_tcav_score time',time.time()-start)
      return float(count) / float(len(class_acts))
  ...
  def get_directional_dir(
    ...
    print('get_directional_dir time',time.time()-start )
  ...
  def run(self, num_workers=4, run_parallel=False, overwrite=False, return_proto=False):
  ...
  def _run_single_set(self, param, overwrite=False, run_parallel=False):
    ...
    # if cav_concept in ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] :
    #   pass
    ...
    del acts
    print('_run_single_set time', time.time()-start)
    return result
  ...
  def _process_what_to_run_expand(self, num_random_exp=100, random_concepts=None):
    ...
    print('_process_what_to_run_expand time',time.time()-start)
        ...
        for alpha in self.alphas:
          ...
          params.append(
              run_params.RunParams(bottleneck, concepts_in_test, target_in_test,
                                   ...
                                   alpha, self.mymodel))
    print('get_params time', time.time()-start)
    return params