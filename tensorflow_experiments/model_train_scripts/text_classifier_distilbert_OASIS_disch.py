...
val_data =  pd.read_csv('../data/oasis_val_disch.csv')
...
#         print(classification_report(self.y_true,y_pred,output_dict=False))
        ...
        print(auc)
        scores_df = scores_df.append({'loss':logs['loss'], 'val_loss':val_loss,'macro_auc':auc},ignore_index=True)
        scores_df.to_csv(source_dir+'checkpoints/distilbert_OASIS_disch'+str(epoch)+'scores.csv')
        try:
            # print(vars(self))
            # print(vars(self.model))
            self.model.save_pretrained(source_dir+'checkpoints/distilbert_OASIS_disch'+str(epoch)+'.h5')
        except:
            print('save failed')
            pass

metrics_callback = MetricsCallback(val_data = np.array([x[0]['input_ids'] for x in list(ds_val_encoded)]), y_true = [x[1] for x in list(ds_val_encoded)])
print('fitting', flush=True)
print('fitting model')

# fit model
batch_size=8
bert_history = model.fit(ds_train_encoded.shuffle(10000).batch(batch_size).repeat(10), epochs=number_of_epochs, validation_data=ds_val_encoded.batch(batch_size), callbacks=[metrics_callback])
