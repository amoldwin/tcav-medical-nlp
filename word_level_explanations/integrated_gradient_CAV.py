...
from saliency_interpreter_CAV import SaliencyInterpreter
...
class IntegratedGradient(SaliencyInterpreter):
    ...
    def saliency_interpret(self, test_dataloader, layer_number,cav):
        ...
        for batch in iterator:
            ...
            # it as attribute:
            self.batch_output = []
            self._integrate_gradients(batch,layer_number=layer_number, cav=cav)
    ...
    def _integrate_gradients(self, batch,layer_number, cav):
        ...
        for alpha in np.linspace(0, 1.0, num=self.num_steps, endpoint=False):
            ...
            grads = self._get_gradients(batch,layer_number=layer_number, cav=cav)
            handle.remove()
