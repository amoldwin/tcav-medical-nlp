...
class SaliencyInterpreter:
    ...
    def _get_gradients(self, batch,layer_number, cav):
        ...
        loss = self.forward_step(batch,layer_number=layer_number, cav=cav)
    ...
    def forward_step(self, batch, layer_number, cav):
        """
...
#         outputs = self.model(input_ids=input_ids, attention_mask=attention_mask, return_dict=True)
        layer_acts = self.model(input_ids=input_ids, return_dict=True,
                          return_logits=False,output_hidden_states=True)['hidden_states'][layer_number]
        cos = torch.nn.CosineSimilarity(dim=1, eps=1e-08)
        cos.requires_grad=True
        outputs = cos(layer_acts.view(layer_acts.size()[0],len(cav)), torch.tensor([cav]*1,device=self.device,requires_grad=True))
        outputs=outputs.view(layer_acts.size()[0],1)
