...
class CAV(object):
    ...
    def load_cav(cav_path):
        ...
        Returns:
          CAV instance.
        """
        with gfile.Open(cav_path, 'rb') as pkl_file:
    ...
    def train(self, acts):
        ...
        logging.info('training with alpha={}'.format(self.hparams['alpha']))
        ...
        self.accuracies, self.metrics_dict = self._train_lm(lm, x, labels, labels2text)
        if len(lm.coef_) == 1:
    ...
    def _save_cavs(self):
        ...
        if self.save_path is not None:
            with gfile.Open(self.save_path, 'w') as pkl_file:
    ...
    def _train_lm(self, lm, x, y, labels2text):
        ...
        num_classes = max(y) + 1
        acc = {}
        metrics_dict = metrics.classification_report(y_test, y_pred)
        num_correct = 0
