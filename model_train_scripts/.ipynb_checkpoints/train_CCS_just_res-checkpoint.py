...
def compute_metrics(eval_pred,  output_dir = output_dir):
    val_eval_pred = (eval_pred[0][0][:len(val_dataset)],eval_pred[1][:len(val_dataset)] )
    test_eval_pred = (eval_pred[0][0][len(val_dataset):],eval_pred[1][len(val_dataset):] )
#     assert False
    epoch=1
    best_mcc=0
    best_ckpt=''
...
model = modeling_bert.WeightedResidualNetwork.from_pretrained("bionlp/bluebert_pubmed_mimic_uncased_L-12_H-768_A-12", weight_list=weight, num_structured_features=num_struct, num_labels=2 )
