...
working_dir = './tcav_class_test'+ckpt.split('/')[-2]+'_'+ckpt.split('/')[-1]+'/'
...
for concept in reversed(concepts):
    negative_concepts = [x for x in os.listdir(concepts_dir) if x.startswith('negative500') and concept.replace('filtered_','') in x and not 'encode' in x]
    print('concept was '+concept+', negative concepts were: ', negative_concepts)
...
#                        num_random_exp=num_random_exp
    ...
    with open(scores_dir+'separate_'+concept +'_'+ ckpt.split('/')[-1]+'_' +str(bottlenecks[0])+'_'+str(num_random_exp)+'randexp.json', 'w') as jfile:
